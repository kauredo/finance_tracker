...
export async function GET(request: NextRequest) {
  try {
    ...
    const suggestions = analyzeTransactions(transactions || [], existingRecurring || [])
...
function analyzeTransactions(transactions: Transaction[], existing: { description: string, amount: number }[]): Suggestion[] {
  ...
  groups.forEach((groupTxs, key) => {
    ...
    // Sort by date desc
    const sortedDates = groupTxs.map(t => new Date(t.date).getTime()).sort((a, b) => b - a)
    ...
    const isAlreadyRecurring = existing.some(e => 
  ...
  return suggestions.sort((a, b) => b.occurrence_count - a.occurrence_count).slice(0, 5)
}
