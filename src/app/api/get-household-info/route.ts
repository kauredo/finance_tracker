...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: household, error } = await supabaseAdmin
      .from('households')
      .select('id, name, household_members(user_id, role, profiles(full_name, email))')
      .eq('id', householdId)
      .single()
    ...
    // Find owner
    const ownerMember = household.household_members?.find((m: any) => m.role === 'owner')
    // profiles is an array because of the join, take the first one
    const profile = Array.isArray(ownerMember?.profiles) ? ownerMember?.profiles[0] : ownerMember?.profiles
